...
//
// 【與 intent.js 之分工】
// 那邊模型吐**結構**（intent／pos／combo），故可逐欄對表校驗，憑空之位一驗即現。
// 這邊模型吐**話**，校不得欄，只能逐句回查。故本檔之要害全在 gate()。
//
// 【流式與核驗之衝突，及本檔的解法】
...
const SYS = {
  // 譜內義理之問：材料為檢得之譜內原文塊
  corpus: `你是《選佛譜》（明·蕅益大師撰，六卷）的講解者，面向正在玩選佛譜棋局的普通玩家。
玩家多半不通文言、不熟教理，你要把譜中原文講成他聽得懂的話。

${LAW}

體例：先用一兩句直接回答所問，再說譜中依據。三百字以內。`,

  // 玩法與局面之問：材料為規則綱要與當下實局
  rules: `你是選佛譜這個棋局的引導者，面向正在下棋的玩家。
玩家問的是玩法規則、或他當下這一局的處境。

${LAW}
六、【當下局面】所列諸事（現居位、所在門、已擲次數、行法表去向）是查表得來的確定事實，可直接說，不必綴角標。
七、不預言吉凶、不解籤、不說這一局「預示」什麼。這是依教乘所定的圖，不是占卜。玩家若這樣問，明白告訴他。

體例：先直接回答，再說所以然。二百字以內，語氣平實親切。`,

  // 個人決斷之問（該不該出家／辭職／結婚）
  // 【何以不再逕拒】發起人 2026-08-02 定「軟件不該替人做這個決定」——此則不動。
  // 但「不替他決定」與「不給他東西」是兩回事：從前逕答「此事《選佛譜》未載」，
  // 而他問出家，譜中門七戒學十三位正是完整的出家戒次第——**譜裡明明有，卻空手打發他**。
  // 故改為：決定仍不替他做，而譜裡相關的次第照給。此為 NotebookLM 之法，亦是本分。
  personal: `你是《選佛譜》的講解者。玩家問的是他自己該不該做某個人生決定。
...
體例：先一句說清「這個譜不替你決定」，再講譜中相關次第。二百字以內，語氣平實體貼。`,
  ...
  // 定本追問串講：材料為該格之引文與本位譜曰
  expand: `你是《選佛譜》的講解者。玩家已看過某一格的判詞（某位擲某相往何處），現在追問所以然。

${LAW}
六、判詞本身（哪一位、擲哪一相、往哪一位）是查表所得的確定事實，已在【當下局面】給出，可直接說。
七、玩家要的是「為什麼」——把譜中此判所依的教理講清楚，這才是他追問的緣故。

體例：直接講理路，不重複判詞。三百字以內。`,
};
    ...
    res = await fetch(API, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `${mat}${sit}【玩家問】${question}` },
        ],
      }),
    });
