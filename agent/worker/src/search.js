...
// shizhu＝大師他著（教觀綱宗・彌陀要解・宗論摘）機切塊——譜內人工塊恆先，他著補其所未詳
const KW = { xu: 1.0, biaofa: 1.3, qa: 1.3, mulu: 0.8, gate: 1.2, table: 0.3, puyue: 1.2, 'puyue-rule': 0.4, end: 0.5, shizhu: 0.9 };
...
const ASK2BOOK = [
  ...
  // 2026-08-14 外典入庫首評補：「和下棋有什么关系」bigram 對不上宗論棋喻塊（善奕者著著皆活）——
  // 大師親口以棋喻法，設計書云宜置於顯處，橋一條把「下棋」翻到那一段
  [/下棋|棋子|象棋|[圍围]棋|棋[藝艺盤盘局]/, ['棋子', '善奕者', '禪宗']],
  // 2026-08-14 修行手冊端到端校出：問「忏悔对治」bigram 懺悔對不上門六三懺塊
  // （塊題作「作法懺」「取相懺」「無生懺」，正文言「說悔」「悔罪」）——橋到三懺本名
  [/[懺忏]悔|[懺忏]法|[懺忏]除|悔[過过罪]|[對对]治/, ['作法懺', '取相懺', '無生懺']],
  [/迷信|算命|封建/, ['選佛譜敘', '教乘']],
  [/人道|人[間间]|做人/, ['人天', '四洲']],
...
export function toPassages(hits) {
  return hits.map((h) => {
    const b = h.block;
    // 大師他著（shizhu 塊）：ref 帶書名與 CBETA 行位，並明標「大師他著」——
    // 紀律三：可引不可混同，前臺出處卡與模型【材料】頭皆見此標
    if (b.src) {
      return {
        title: `${b.src}（大師他著）`,
        text: b.t,
